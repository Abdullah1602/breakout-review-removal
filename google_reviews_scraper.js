// Load .env for local development (ignored on Heroku)
...
async function scrollToBottom(page, maxReviews = 200) {
  let lastCount = 0;
  let unchanged = 0;
  ...
  while (true) {
    ...
    if (newCount >= maxReviews) {
      console.log(`Reached max review limit (${maxReviews}). Stopping scroll to save memory.`);
      break;
    }
